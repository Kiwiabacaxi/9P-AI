...
const percPortasAlfa = 0.01
const percPortasMaxCiclos = 1000
...
func percPortasBipolar(soma float64) int {
	if soma >= 0 {
		return 1
	}
	return -1
...
func percPortasTreinar(p PortaLogica) PercPortasResult {
	...
	for ciclo < percPortasMaxCiclos {
		...
		for i := 0; i < 4; i++ {
			...
			y := percPortasBipolar(yLiq)
			teveErro := false

			if y != p.Targets[i] {
				errou = true
				teveErro = true
				delta := percPortasAlfa * float64(p.Targets[i]-y)
				w1 += delta * float64(x1)
				w2 += delta * float64(x2)
				bias += delta
			}
