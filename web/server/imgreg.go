...
// ImgRegConfig — parâmetros configuráveis pelo usuário
type ImgRegConfig struct {
...
// ImgRegStep — mensagem enviada via SSE durante e ao fim do treino.
...
// imgregInicializar — cria uma nova rede com arquitetura [2, N, N, ..., 3]
...
func imgregInicializar(cfg ImgRegConfig, rng *rand.Rand) ImgRegNet {
	...
	return ImgRegNet{LayerSizes: sizes, W: W, B: B}
}
...
func imgregForward(net ImgRegNet, x, y float64) (activations [][]float64, preActivations [][]float64) {
...
//   - gradB[l][j]    = ∂L/∂B[l][j]
func imgregBackward(net ImgRegNet, activations [][]float64, preActivations [][]float64, target [3]float64) (gradW [][][]float64, gradB [][]float64) {
...
// imgregAtualizarPesos — aplica SGD (Stochastic Gradient Descent):
// W[l][i][j] -= α * gradW[l][i][j]
// B[l][j]    -= α * gradB[l][j]
// O sinal negativo segue a direção do gradiente descendente (minimizar loss)
func imgregAtualizarPesos(net ImgRegNet, gradW [][][]float64, gradB [][]float64, lr float64) ImgRegNet {
	for l := range net.W {
		for i := range net.W[l] {
...
// imgGetTarget — retorna os 256 pixels RGB [0,1] da imagem escolhida
// Os pixels são armazenados em ordem row-major: pixel[y*16 + x]
func imgGetTarget(nome string) [][3]float64 {
	switch nome {
	case "smiley":
		return imgSmiley()
	case "radial":
...
func imgCoracao() [][3]float64 {
	...
	for py := 0; py < 16; py++ {
		for px := 0; px < 16; px++ {
			// Normaliza coordenadas para [-1.5, 1.5]
			x := (float64(px)/15.0)*3.0 - 1.5
			y := -((float64(py)/15.0)*3.0 - 1.5) // inverte Y (tela vs matemática)

			// Equação do coração: (x²+y²-1)³ ≤ x²y³
			x2, y2 := x*x, y*y
			f := (x2+y2-1)*(x2+y2-1)*(x2+y2-1) - x2*y*y2
...
// imgregPredict — aplica a rede a todos os 256 pixels e retorna as predições RGB
func imgregPredict(net ImgRegNet) [][3]float64 {
	pixels := make([][3]float64, 256)
	for py := 0; py < 16; py++ {
		for px := 0; px < 16; px++ {
...
// imgregTreinar — loop de treinamento principal com streaming SSE
// Cada época = apresentar os 256 pixels em ordem aleatória (SGD com shuffle)
// Envia ImgRegStep pelo canal a cada epochStep épocas para atualizar o frontend
func imgregTreinar(cfg ImgRegConfig, progressCh chan<- ImgRegStep) ImgRegNet {
	...
	net := imgregInicializar(cfg, rng)

	// Carrega a imagem-alvo (256 pixels RGB)
	target := imgGetTarget(cfg.Imagem)
	...
	for epoca := 1; epoca <= cfg.MaxEpocas; epoca++ {
		...
		for _, idx := range indices {
			...
			net = imgregAtualizarPesos(net, gradW, gradB, cfg.LearningRate)
		}
	...
	// Envia mensagem final com Done=true — dentro da goroutine antes do close,
	// garante que o frontend receba o resultado sem race condition.
	finalPixels := imgregPredict(net)
	convergiu := lossHistorico[len(lossHistorico)-1] < 0.01
	progressCh <- ImgRegStep{
		Done:          true,
		Convergiu:     convergiu,
